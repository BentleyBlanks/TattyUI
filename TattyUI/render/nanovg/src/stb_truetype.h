...
////   of C library functions used by stb_truetype.
   ...
   typedef unsigned int    stbtt_uint;
   typedef signed   int    stbtt_int;
   #endif

   typedef char stbtt__check_size32[sizeof(stbtt_int)==4 ? 1 : -1];
...
#define ttFixed(p)    ttLONG(p)
   ...
   #define ttULONG(p)    (* (stbtt_uint *) (p))
   #define ttLONG(p)     (* (stbtt_int *) (p))

#else

   stbtt_uint16 ttUSHORT(const stbtt_uint8 *p) { return p[0]*256 + p[1]; }
   stbtt_int16 ttSHORT(const stbtt_uint8 *p)   { return p[0]*256 + p[1]; }
   stbtt_uint ttULONG(const stbtt_uint8 *p)  { return (p[0]<<24) + (p[1]<<16) + (p[2]<<8) + p[3]; }
   stbtt_int ttLONG(const stbtt_uint8 *p)    { return (p[0]<<24) + (p[1]<<16) + (p[2]<<8) + p[3]; }
...
static stbtt_uint stbtt__find_table(stbtt_uint8 *data, stbtt_uint fontstart, const char *tag)
{
   stbtt_int num_tables = ttUSHORT(data+fontstart+4);
   stbtt_uint tabledir = fontstart + 12;
   stbtt_int i;
   for (i=0; i < num_tables; ++i) {
      stbtt_uint loc = tabledir + 16*i;
...
int stbtt_GetFontOffsetForIndex(const unsigned char *font_collection, int index)
{
   ...
   if (stbtt_tag(font_collection, "ttcf")) {
      ...
      if (ttULONG(font_collection+4) == 0x00010000 || ttULONG(font_collection+4) == 0x00020000) {
         stbtt_int n = ttLONG(font_collection+8);
         if (index >= n)
            return -1;
...
int stbtt_InitFont(stbtt_fontinfo *info, const unsigned char *data2, int fontstart)
{
   ...
   for (i=0; i < numTables; ++i) {
      stbtt_uint encoding_record = cmap + 4 + 8 * i;
...
int stbtt_FindGlyphIndex(const stbtt_fontinfo *info, int unicode_codepoint)
{
   ...
   stbtt_uint index_map = info->index_map;

   stbtt_uint16 format = ttUSHORT(data + index_map + 0);
   if (format == 0) { // apple byte encoding
      stbtt_int bytes = ttUSHORT(data + index_map + 2);
      if (unicode_codepoint < bytes-6)
         return ttBYTE(data + index_map + 6 + unicode_codepoint);
      return 0;
   } else if (format == 6) {
      stbtt_uint first = ttUSHORT(data + index_map + 6);
      stbtt_uint count = ttUSHORT(data + index_map + 8);
      if ((stbtt_uint) unicode_codepoint >= first && (stbtt_uint) unicode_codepoint < first+count)
      ...
      stbtt_uint endCount = index_map + 14;
      stbtt_uint search = endCount;
      ...
      stbtt_uint ngroups = ttULONG(data+index_map+12);
      stbtt_int low,high;
      low = 0; high = (stbtt_int)ngroups;
      // Binary search the right group.
      while (low < high) {
         stbtt_int mid = low + ((high-low) >> 1); // rounds down, so low <= mid < high
         stbtt_uint start_char = ttULONG(data+index_map+16+mid*12);
         stbtt_uint end_char = ttULONG(data+index_map+16+mid*12+4);
         if ((stbtt_uint) unicode_codepoint < start_char)
            high = mid;
         else if ((stbtt_uint) unicode_codepoint > end_char)
            low = mid+1;
         else {
            stbtt_uint start_glyph = ttULONG(data+index_map+16+mid*12+8);
            if (format == 12)
...
static void stbtt_setvertex(stbtt_vertex *v, stbtt_uint8 type, stbtt_int x, stbtt_int y, stbtt_int cx, stbtt_int cy)
{
   v->type = type;
   v->x = (stbtt_int16) x;
   v->y = (stbtt_int16) y;
...
static int stbtt__close_shape(stbtt_vertex *vertices, int num_vertices, int was_off, int start_off,
    stbtt_int sx, stbtt_int sy, stbtt_int scx, stbtt_int scy, stbtt_int cx, stbtt_int cy)
{
   if (start_off) {
      if (was_off)
...
int stbtt_GetGlyphShape(const stbtt_fontinfo *info, int glyph_index, stbtt_vertex **pvertices)
{
   ...
   *pvertices = NULL;
   ...
   if (numberOfContours > 0) {
      ...
      stbtt_int ins, i,j=0,m,n, next_move, was_off=0, off, start_off=0;
      stbtt_int x,y,cx,cy,sx,sy, scx,scy;
      stbtt_uint8 *points;
         ...
         if (next_move == i) {
            ...
            if (start_off) {
               ...
               if (!(vertices[off+i+1].type & 1)) {
                  ...
                  sx = (x + (stbtt_int) vertices[off+i+1].x) >> 1;
                  sy = (y + (stbtt_int) vertices[off+i+1].y) >> 1;
               } else {
                  // otherwise just use the next point as our start point
                  sx = (stbtt_int) vertices[off+i+1].x;
                  sy = (stbtt_int) vertices[off+i+1].y;
...
static stbtt_int stbtt__CompareUTF8toUTF16_bigendian_prefix(const stbtt_uint8 *s1, stbtt_int len1, const stbtt_uint8 *s2, stbtt_int len2) 
{
   stbtt_int i=0;
...
{
   stbtt_int i,count,stringOffset;
   stbtt_uint8 *fc = font->data;
   stbtt_uint offset = font->fontstart;
   stbtt_uint nm = stbtt__find_table(fc, offset, "name");
   if (!nm) return NULL;

   count = ttUSHORT(fc+nm+2);
   stringOffset = nm + ttUSHORT(fc+nm+4);
   for (i=0; i < count; ++i) {
      stbtt_uint loc = nm + 6 + 12 * i;
...
static int stbtt__matchpair(stbtt_uint8 *fc, stbtt_uint nm, stbtt_uint8 *name, stbtt_int nlen, stbtt_int target_id, stbtt_int next_id)
{
   stbtt_int i;
   stbtt_int count = ttUSHORT(fc+nm+2);
   stbtt_int stringOffset = nm + ttUSHORT(fc+nm+4);

   for (i=0; i < count; ++i) {
      stbtt_uint loc = nm + 6 + 12 * i;
      stbtt_int id = ttUSHORT(fc+loc+6);
      if (id == target_id) {
         // find the encoding
         stbtt_int platform = ttUSHORT(fc+loc+0), encoding = ttUSHORT(fc+loc+2), language = ttUSHORT(fc+loc+4);

         // is this a Unicode encoding?
         if (platform == 0 || (platform == 3 && encoding == 1) || (platform == 3 && encoding == 10)) {
            stbtt_int slen = ttUSHORT(fc+loc+8);
            stbtt_int off = ttUSHORT(fc+loc+10);

            // check if there's a prefix match
            stbtt_int matchlen = stbtt__CompareUTF8toUTF16_bigendian_prefix(name, nlen, fc+stringOffset+off,slen);
            if (matchlen >= 0) {
...
static int stbtt__matches(stbtt_uint8 *fc, stbtt_uint offset, stbtt_uint8 *name, stbtt_int flags)
{
   stbtt_int nlen = (stbtt_int) STBTT_strlen((char *) name);
   stbtt_uint nm,hd;
...
int stbtt_FindMatchingFont(const unsigned char *font_collection, const char *name_utf8, stbtt_int flags)
{
   stbtt_int i;
   for (i=0;;++i) {
      stbtt_int off = stbtt_GetFontOffsetForIndex(font_collection, i);
      if (off < 0) return off;
