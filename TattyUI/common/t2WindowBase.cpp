...
// 绝对坐标
static int amouseX = 0, amouseY = 0;
static int apmouseX = 0, apmouseY = 0;

static double mouseX = 0, mouseY = 0;
// 上一帧鼠标位置
static double pmouseX = 0, pmouseY = 0;
...
namespace TattyUI
{
    t2WindowBase::t2WindowBase() :radius(10)
	{
		// 挂钩错误处理回调函数
		glfwSetErrorCallback(error_cb);
	}

	void t2WindowBase::setup(int width, int height)
	{
		if (!glfwInit())
		{
			cout << "t2WindowBase:" << "无法初始化 glfw" << endl;
			return;
		}

        // 配置OpenGL版本
        // --!Mac下某些机型上的OSX只支持到OpenGL 3.x 那么可以在t2WindowBase.setup之前定义glversionMajor与glVersionMinor
		...
		if (mode == T2_WINDOW_GAMEMODE)
		{
			...
			{
				cout << "t2WindowBase:" << "无法找到任何显示器" << endl;
			}
		}
		else
		{
			if (mode == T2_WINDOW_NOTITLE)
			{
				...
				if (!glfwWindow)
				{
					cout << "t2WindowBase:" << "无法创建glfw窗口" << endl;
				}
	...
	// getter/setter
	// --!get
	t2Point2i t2WindowBase::getSize()
	{
		if (mode == T2_WINDOW_GAMEMODE)
		{
	...
	t2Point2i t2WindowBase::getPosition()
	{
        t2Point2i temp;

        glfwGetWindowPos(glfwWindow, &temp.x, &temp.y);
        //temp.print();
	...
	int t2WindowBase::getHeight()
	{
		return getSize().y;
	}

	int t2WindowBase::getWidth()
	{
		return getSize().x;
	}

    std::string t2WindowBase::getTitle()
    {
        return title;
    }

	bool t2WindowBase::isVerticalSync()
	{
		return verticalSync;
	}

	int t2WindowBase::getMode()
	{
		return mode;
	}

	t2Point2i t2WindowBase::getScreenSize()
	{
		int count;
    ...
    void t2WindowBase::setRadius(int raidus)
    {
        this->radius = raidus;
    }

	// --!set
	void t2WindowBase::setPosition(int x, int y)
	{
		setPosition(t2Point2i(x, y));
	}

	void t2WindowBase::setPosition(t2Point2i position)
	{
		this->position.clone(position);
		glfwSetWindowPos(glfwWindow, position.x, position.y);
	}

	void t2WindowBase::setSize(int width, int height)
	{
		setSize(t2Point2i(width, height));
	}

	void t2WindowBase::setSize(const t2Point2i size)
	{
	...
	void t2WindowBase::setResizable(bool resizable)
	{
		this->resizable = resizable;
	}

	void t2WindowBase::setHeight(int height)
	{
		this->height = height;
	...
	void t2WindowBase::setWidth(int width)
	{
		this->width = height;
	...
	void t2WindowBase::setMode(int mode)
	{
		this->mode = mode;
	}

	void t2WindowBase::setVeticalSync(bool verticalSync)
	{
	...
	void t2WindowBase::setTitle(std::string title)
	{
		this->title = title;
		glfwSetWindowTitle(glfwWindow, title.c_str());
	}

	void t2WindowBase::setIcon()
	{
		// --!暂无图片支持
	}

	void t2WindowBase::set2d(float width, float height)
	{
		//设置视区尺寸
	...
	void t2WindowBase::setFullScreen()
	{
		HWND hwnd = glfwGetWin32Window(glfwWindow);
	...
	void t2WindowBase::swapBuffer()
	{
		// 交换前后两缓冲 
		glfwSwapBuffers(glfwWindow);
	}

	void t2WindowBase::processEvents()
    {
        t2UpdateAbsoluteMousePos();
	...
	// 挂钩所有回调函数
	void t2WindowBase::initCallBack()
	{
		if (!glfwWindow)
			return;
        ...
        //glfwSetWindowPosCallback(glfwWindow, windowpos_cb);
	...
	// 获得当前焦点所在的窗口的序号
	int t2WindowBase::getCurrentMonitor()
	{
		int monitorCount;
		GLFWmonitor** monitors = glfwGetMonitors(&monitorCount);
    ...
    bool t2WindowBase::initGlew()
    {
        ...
        if(GLEW_OK != err)
        {
            ...
            t2Log("t2WindowBase: couldn't init GLEW: %s", glewGetErrorString(err));
            return false;
        }
...
static void windowpos_cb(GLFWwindow *, int, int)
{
...
{
    TattyUI::t2Point2i t(amouseX, amouseY);
    t.print();
